...
// Imports des composants de notification (assure-toi d'avoir ces fichiers au chemin indiqué)
import { NotificationProvider } from "@/app/component/NotificationProvider";
import { NotificationButton } from "@/app/component//NotificationButton";
import { NotificationPopup } from "@/app/component/NotificationPopup";
import {RealtimeNotifications} from "@/app/component/RealtimeNotifications";
    ...
    <NotificationProvider>
      <div className="min-h-screen flex bg-gradient-to-br from-gray-50 to-gray-100">
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center gap-3 p-3 rounded-xl transition-all duration-300 ${
                    active ? "bg-white/20 backdrop-blur-sm shadow-lg" : "hover:bg-white/10 hover:translate-x-1"
                  }`}
                >
        ...
        <main className="flex-1 flex flex-col">
          {/* Header (make it relative so the popup can be positioned against it) */}
          <header className="bg-white shadow-sm border-b border-gray-200 relative">
            <div className="flex justify-between items-center px-8 py-6">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Bienvenue, {user.name || "Client"}</p>
              </div>

              <div className="flex items-center gap-4">
                {/* Notifications: replace the previous button with NotificationButton */}
                <div className="relative">
                  {/* Notification button (toggles popup) */}
                  <NotificationButton className="relative p-3 hover:bg-gray-100 rounded-xl transition-all group" />

                  {/* Notification popup anchored to header */}
                  <NotificationPopup />
                </div>
                ...
                <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 text-sm">{user.name || "Client"}</p>
          ...
          {/* Realtime socket initializer (no UI) */}
